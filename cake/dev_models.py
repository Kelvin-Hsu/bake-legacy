...
from .kernels import s_gaussian_mlp2 as _s_gaussian_mlp2
...
class DKEC():
    ...
    def __init__(self, kernel=_s_gaussian_mlp2):
        """
    ...
    def fit(self, x, y,
        ...
        d_1 = 5
        d_2 = 4
        w_1 = 2 * np.random.rand(self.d, d_1) - 1
        b_1 = 2 * np.random.rand(d_1) - 1
        w_2 = 2 * np.random.rand(d_1, d_2) - 1
        b_2 = 2 * np.random.rand(d_2) - 1
        self.w_1 = tf.Variable(w_1)
        self.b_1 = tf.Variable(b_1)
        self.w_2 = tf.Variable(w_2)
        self.b_2 = tf.Variable(b_2)
        ...
        if log_hypers:
            ...
            flat_var_list = [self.log_theta, self.log_zeta,
                             tf.reshape(self.w_1, [-1]), self.b_1,
                             tf.reshape(self.w_2, [-1]) , self.b_2]
        else:
            ...
            var_list = [self.theta, self.zeta,
                        self.w_1, self.b_1, self.w_2, self.b_2]
            flat_var_list = [self.theta, self.zeta,
                             tf.reshape(self.w_1, [-1]), self.b_1,
                             tf.reshape(self.w_2, [-1]) , self.b_2]
        ...
        self.grad_norm = [tf.reduce_max(tf.abs(grad)) for grad in self.grad]
        ...
        if train:
            while grad_norm_check > grad_tol:
                ...
                grad_norm = np.max(self.sess.run(self.grad_norm,
                                                 feed_dict=feed_dict))
                grad_norm_check = grad_norm
                self.sess.run(train, feed_dict=feed_dict)
                print('Step %d' % step,
                      ...
                      '|| Cross Entropy Loss: ', cel,
                      '|| Gradient Norm: ', grad_norm)
                self.w_1_train = self.sess.run(self.w_1)
                self.b_1_train = self.sess.run(self.b_1)
                self.w_2_train = self.sess.run(self.w_2)
                self.b_2_train = self.sess.run(self.b_2)
                print('Weight Matrix 1: \n', self.w_1_train)
                print('Bias Vector 1: ', self.b_1_train)
                print('Weight Matrix 2: \n', self.w_2_train)
                print('Bias Vector 2: ', self.b_2_train)
                step += 1
        ...
        self.w_2_train = self.sess.run(self.w_2)
        self.b_2_train = self.sess.run(self.b_2)
        print('Learned Weight Matrix 1: \n', self.w_1_train)
        print('Learned Bias Vector 1: ', self.b_1_train)
        print('Learned Weight Matrix 2: \n', self.w_2_train)
        print('Learned Bias Vector 2: ', self.b_2_train)
        return self
