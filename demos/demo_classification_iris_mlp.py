...
def load_iris_data(a, b, normalize_features=True):
    ...
    -------
    numpy.ndarray
        The features (n_train, 2)
    numpy.ndarray
        The target labels (n_train, 1)
...
def iris_classification(x_train, y_train, x_test, y_test,
    ...
    grad_tol = 0.1
    n_sgd_batch = None
    kec = cake.DKEC().fit(x_train, y_train, theta=theta_init, zeta=zeta_init,
    ...
    kec_h_query_alt = kec_h_query # kec.predict_entropy(x_query)
    ...
    reverse_weights = kec.reverse_weights(y_query) # (n_train, 3)
    ...
    for c, image in enumerate(reverse_embedding_images):
        ...
        plt.scatter(x_rep[c, 0], x_rep[c, 1], c=y_rep[c], marker='x_train', s=40,
    ...
    #                       'train_cross_entropy_loss': _train_history[3, :],
    ...
    plt.plot(th['iterations'], th['train_cross_entropy_loss'], c='b', linestyle='--',
    ...
    f = open('%s%s_results.txt' % (directory, name), 'train_w')
