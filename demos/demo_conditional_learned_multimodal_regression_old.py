...
def main():
    ...
    mu_yx_optimal = bake.infer.conditional_embedding(x, y, theta_x, theta_y, zeta)

    x_lim = np.max(np.abs(x))
    y_lim = np.max(np.abs(y)) + 1.0
    xq_array = np.linspace(-x_lim, x_lim, 300)
    yq_array = np.linspace(-y_lim, y_lim, 300)
    ...
    n_modes = 10
    y_modes = bake.infer.conditional_modes(mu_yx_optimal, xq, yv_min, yv_max, theta_y, n_modes = n_modes)
    ...
    plt.scatter(xv[:n_modes, :].ravel(), y_modes.ravel(), s = 20, c = 'w', edgecolor = 'face')
    plt.xlim((-x_lim, x_lim))
    plt.ylim((-y_lim, y_lim))
    plt.xlabel('$x$')
    plt.ylabel('$y$')
