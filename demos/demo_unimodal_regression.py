...
def main():
    ...
    # Conditional weights
    wq = bake.infer.conditional_weights(x, y, theta_x, theta_y, xq, zeta=0)
    mu_yqxq = bake.infer.evaluate_embedding(yq, y, theta_y, wq)

    # Expectance and Covariance
    yq_exp = bake.infer.expectance(y, wq)[0]
    yq_cov = bake.infer.covariance(y, wq)[0]
    yq_var = yq_cov.diagonal()
    yq_std = np.sqrt(yq_var)
    print(yq_exp.shape, yq_std.shape)
